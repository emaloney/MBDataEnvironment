...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -workspace MBDataEnvironment.xcworkspace -configuration Debug -scheme "MBDataEnvironment" -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "MBDataEnvironment-$PLATFORM-$OPERATION.log" | xcpretty )
	XCODE_RESULT="${PIPESTATUS[0]}"
	if [[ "$XCODE_RESULT" == "0" ]]; then
		rm "MBDataEnvironment-$PLATFORM-$OPERATION.log"
		exit 0
