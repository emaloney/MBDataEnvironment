...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project MBDataEnvironment.xcodeproj -configuration Debug -scheme "MBDataEnvironment" -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "MBDataEnvironment-$PLATFORM-$OPERATION.log" | xcpretty )
